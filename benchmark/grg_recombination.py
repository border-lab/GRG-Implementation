...
class NonDuplicationRecombination:
    ...
    def _recurse_attach(
        self, node_id, offspring_id, L, R, visited=None, connected=None
    ):
        if L >= R:
            return
        ...
        if Iu is None:
            ...
            if has_partial_relevant:
                ...
                bubble_id = self._extract_bubble(
                    node_id, rel_mut_ids, offspring_id, (L, R)
                )
                connected.add(bubble_id)
            return
        ...
        if has_all_relevant and full_coverage:
            if self.debug_mode:
                print(
                    "Case 1: Full coverage with all relevant mutations - connect directly"
                )
            # if (node_id not in connected and
            #     not self._has_connected_descendant(node_id, connected)):
            if node_id not in connected:
                ...
                print(
                    f"Node {node_id} already connected or has connected relatives, skipping direct connection"
                )
            return  # Stop - lineage fully resolved
        ...
        if has_no_relevant and not ancestral_disjoint:
            if self.debug_mode:
                print(
                    "Case 3: No relevant mutations but not disjoint ancestry - path compression"
                )
            ...
            if (
                L >= R
            ):  # avoids useless parent calls on degenerate overlap at boundaries
                return
        ...
        if has_partial_relevant or has_all_relevant:
            if self.debug_mode:
                print(
                    "Case 4: Partial/full relevant mutations - need to create bubble and maybe recurse upwards"
                )
    ...
    def recombine(self, haplotype_A, haplotype_B, breakpoint):
        ...
        self._recurse_attach(
            haplotype_A, offspring_id, 0, breakpoint, visited=None, connected=connected
        )
